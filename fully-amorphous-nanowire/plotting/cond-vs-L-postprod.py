...
palette = seaborn.color_palette(palette='magma', n_colors=G_array.shape[2])
...
for i in range(G_array.shape[0]):
    ...
    for j in range(G_array.shape[2]):
        ...
        ax.plot(Nz[::-1], G_array[i, :, j][::-1], color=palette[j], linestyle='solid', marker=marker_list[j], label=label)
