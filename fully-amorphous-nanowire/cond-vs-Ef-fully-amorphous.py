...
width            = [0.00001, 0.02, 0.05, 0.1] # Spread of the Gaussian distribution for the lattice sites
...
fermi            = np.linspace(0, 1, 300)     # Fermi energy
K_hopp           = 0.
K_onsite         = 0.
...
for j, w in enumerate(width):
    ...
    flux_max[j], Gmax = select_perfect_transmission_flux(nanowire)
