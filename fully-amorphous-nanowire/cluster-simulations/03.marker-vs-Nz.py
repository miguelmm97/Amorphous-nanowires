...
Ny         = 10
r          = 1.3
width      = np.linspace(0.00001, 0.9, 20)
t          = 1
eps        = 4 * t
lamb       = 1 * t
lamb_z     = 1.8 * t
...
# Preallocation
Nsites = int(np.max(Nz) * Nx * Ny)
marker = np.zeros((len(width), len(Nz), Nsites))
X = np.zeros((len(width), len(Nz), Nsites))
Y = np.zeros((len(width), len(Nz), Nsites))
Z = np.zeros((len(width), len(Nz), Nsites))
...
for i, w in enumerate(width):
    for j, n in enumerate(Nz):
        ...
        lattice = AmorphousLattice_3d(Nx=Nx, Ny=Nx, Nz=n, w=w, r=r)
        ...
        loger_main.info(f'width: {i}/{len(width) - 1}, Nx: {j}/{len(Nz) - 1}')
